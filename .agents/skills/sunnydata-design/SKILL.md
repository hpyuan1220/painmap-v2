...
> **Note:** Phase 3 works significantly better with subagent support (e.g., Codex or Codex). If subagents are available, prefer subagent-driven execution: dispatch a fresh subagent per task with two-stage review between tasks. Phase 3 inline execution is the fallback for environments without subagent support.
